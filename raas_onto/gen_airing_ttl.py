...
raas:ContinuousSchedulingAxiom a raas:DomainAxiom ;
    ...
    rdfs:comment "편성형 라디오 채널은 24시간 빈 시간 없이 연속 편성된다. 어떤 시각의 방송분을 구하는 절차: (1) 현행 프로그램을 start_time 오름차순으로 정렬한다. (2) 각 프로그램은 자기 start_time부터 '바로 다음 프로그램의 start_time 직전'까지 방송된다(그 사이 여러 시간대를 커버). (3) 따라서 시각 H의 방송분 = start_time ≤ H 인 프로그램 중 start_time이 가장 늦은(직전) 프로그램이다. **H보다 늦게 시작하는 프로그램은 절대 H를 커버하지 않는다.** 빈 시간대는 뒤 프로그램이 아니라 '직전(앞) 프로그램의 연장'으로 채운다. 워크드 예시: 현행이 04:00 OLDIES 20(재), 06:00 고현준의 뉴스 브리핑이고 05:00 시작 프로그램이 없으면 → 05:00은 OLDIES 20(재)(04:00 시작, 06:00 직전까지 방송)이다. 06:00 고현준(05:00보다 늦게 시작)은 05:00을 커버하지 않으며, '05시가 06시로 흡수'가 아니라 '05시는 04시 프로그램의 연장'이다. 종영 이력에서 어떤 시간대의 마지막 프로그램이 종영한 뒤 그 시각에 새 시작 프로그램이 없다면, 그 시간대는 직전(더 이른 시각) 프로그램이 길어져 흡수한 것으로 해석하라."@ko .
