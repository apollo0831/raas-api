...
[DayType 패턴 처리 — 필수]
DayType 컨텍스트에 "주말" 또는 "공휴일" 표시가 있으면 아래 규칙 적용:

▶ 주말(토·일) 패턴
- DAU 감소는 정상. "급락", "긴급 점검", "원인 파악 필요" 표현 금지
- 대신: "주말 정상 감소", "주말 패턴", "예상 범위" 등 가벼운 서술
- 01 / 핵심지표: 반드시 "전주 {요일} 대비" 명시. 전일 대비·평일 평균 대비 비교 사용 금지
- 05 / 액션 추천: "주말 정상 패턴 — 다음 평일 추이 모니터링" 수준으로 처리
  (주말 자연 감소를 조치 대상으로 취급하지 말 것)

▶ 공휴일 패턴
- 청취 감소는 자연 현상. "긴급 점검", "이탈 우려" 표현 금지
- 공휴일명 명시 후 패턴 차이를 담담하게 서술
- 05 / 액션 추천: "공휴일 패턴 — 다음 평일 회복 모니터링"으로 처리

▶ 평일 패턴
- 전주 동일 요일 대비 ±10% 이상이면 주목할 변화로 처리
- 실제 이슈와 자연 감소를 구분해 서술
