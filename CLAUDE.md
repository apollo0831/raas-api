...
raas_server.py  ← 진입점, ThreadingHTTPServer on port 5000
    ...
    │          실시간(동시사용자) 의도면 _build_extract_realtime로 분기 — 행=1분, 열=채널
    │          (전체·파워FM·러브FM·고릴라M·픽채널). 날짜 지정=그 날(보관 범위 내)/없으면 오늘,
    │          월·기간범위는 미지원 사유 반환(row_label='분')
...
- `raas_planner.py` [P-1a] — LLM 질의 플래너(shadow, **프로덕션 미연결**). 질의→검증된
  ...
  **값은 안 만듦**(실행기·접근자가 계산). 흩어진 감지기 19종을 대체할 토대. 평가:
  ...
  실패·저신뢰는 키워드 spec 폴백.
  [P-1d] 텔레메트리(`planner_stats`/`_record_planner`) — 플래너 채택/폴백을 confidence와 기록,
  `GET /api/planner/stats`(관리자)로 모니터링(실트래픽 관찰→확대 판단). **경계**: series/compare/
  general은 provider 번들(program_kpi·metric_timeseries·flow·cohort…)이 풍부해 focused 실행기로
  줄이면 퇴화 → 기존 select_providers(이미 LLM) 경로 유지. 키워드 감지기는 플래너 커버 scope에서
  **폴백 안전망**으로 존속(삭제 안 함).
